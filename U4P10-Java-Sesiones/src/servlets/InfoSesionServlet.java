...
public class InfoSesionServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("reiniciarSesion") != null) {
			session.invalidate();
			response.sendRedirect("/U4P10-Java-Sesiones/InfoSesionServlet");
			// session = request.getSession();
		} else {

			if (session.isNew()) {
				session.setAttribute("contador", 0);
				session.setAttribute("primeraVez", true);
			...
			out.println("<h2>Información sobre la sesión</h2>" + "<ul>" + "<li> Identificador: " + session.getId()
					+ "</li>\n" + "<li> Fecha de creación: " + fechaInicioSesion + "</li>\n"
					+ "<li> Fecha de último acceso: " + fechaUltimoAcceso + "</li>\n" + "<li> Número de visitas: "
					+ contador + "</li>\n" + "</ul>" + "<p><a href='" + request.getRequestURI()
					+ "'>Refrescar</a></p>");
			...
			if (session.getAttribute("primeraVez") != null) {
				out.println("<p>Esta es tu primera visita.</p>");
			}
